...
def interpreter(data):
    ...
    if str_list[0]=="LOGIN":
        ...
        accounts_file.close()
        return weather
    elif str_list[0] == "CALENDAR":
        change_something(username,8,"1")
        calendar_thread = threading.Thread(target = open_calendar,name=calendar)
        calendar_thread.daemon = True
        calendar_thread.start()
        return "1"
    elif str_list[0] == "NOCALENDAR":
        change_something(username,8,"0")
        os.system("sudo pkill -f agenda")
        return "1"
    elif str_list[0] == "SETNEWS":
        accounts_file = open("Accounts.txt",'r')
        for line in accounts_file:
            split_line = line.split(",")
            if(username == split_line[0]):
        ...
        accounts_file.close()
        return news
    elif str_list[0] == "SETCALENDAR":
        accounts_file = open("Accounts.txt",'r')
        for line in accounts_file:
            split_line = line.split(",")
            if(username == split_line[0]):
                calendar = split_line[8]
                break
        accounts_file.close()
        return calendar
    else:  
...
def open_calendar():
    os.system("sudo pkill -f agenda")
    os.system("python3 /home/pi/SmartMirror/agenda.py")
    ...
    for line in accounts_file:
        ...
        if(username == split_line[0]):
            ...
            if(split_line[5] == "1"):
                ...
                news_thread.start()
            if(split_line[8] == "1"):
                calendar_thread = threading.Thread(target = open_calendar,name=open_calendar)
                calendar_thread.daemon = True
                calendar_thread.start()
                
            #os.system("do sound shit" + split_line[6])
            #os.system("do light shit" + split_line[7])
    return
